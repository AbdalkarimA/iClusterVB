...
Rcpp::List CAVI_algorithm_global(Rcpp:: List mydata,               // List of R datasets (R is the number of outcome, R>=2)
  ...
  //Rcpp::Environment MCMCpack("package:MCMCpack");
      //Function ddirichlet_glo = MCMCpack["ddirichlet_glo"];
  ...
  while (true) {
    ...
    for (int i = 0; i < N; ++i) {
      phi.row(i) =   softmax_log_glo(vectorise(log_phi.row(i))).t() ;
    }
        ...
        for (int r = 0; r < R; ++r) {
          ...
          //---------------------------------------//
            if (dist(r) == "gaussian") {
              ...
              arma:: vec indx_tmp = findIndices_char_glo(dist, "gaussian") ;
              int indx =  findIndex_numeric_glo(indx_tmp, r);
            ...
            if(dist(r) == "multinomial"){
              arma:: vec indx_tmp = findIndices_char_glo(dist, "multinomial") ;
              int indx =  findIndex_numeric_glo(indx_tmp, r);
             ...
             if(dist(r) == "poisson"){
               arma:: vec indx_tmp = findIndices_char_glo(dist, "poisson") ;
               int indx =  findIndex_numeric_glo(indx_tmp, r);
